...
## 14.3 通知类型
![通知类型，Around最常用](Developer_images/73.png)   
![样例代码](Developer_images/74.png)
![通知小结](Developer_images/75.png)
![通知顺序](Developer_images/76.png)

## 14.4 切点表达式
![切点表达式——execution，基于方法](Developer_images/77.png)   
![切入点小结](Developer_images/78.png)   
![切点表达式——annotation,基于注解](Developer_images/79.png)

## 14.5 连接点
![连接点](Developer_images/80.png)
![连接点joinpoint示例](Developer_images/81.png)   
连接点proceedingJoinPoint示例，与上面用法一样，只是前面设定是为proceedingJoinPoint

## 14.6 实战
![增删改操作记录进入日志，用的是注解表达式](Developer_images/82.png)    
![需要新创建实体类log，并写入数据库的mapper，引用mapper（@autowired）](Developer_images/83.png)    

## 14.7 线程
![线程——threadlocal](Developer_images/84.png)   
![流程示例](Developer_images/85.png)   
![加上一个工具类](Developer_images/86.png)   
![在token中提取相关id信息，这个线程中始终保存着](Developer_images/87.png)
![重写之前getid的方法，利用缓存map来提取id信息](Developer_images/88.png)

## 15 SpringBoot原理
## 15.1 配置优先级
![配置优先级，yml为主流](Developer_images/89.png) 
![配置优先级](Developer_images/90.png)  

## 15.2 bean管理
![bean作用域](Developer_images/91.png) 
![bean作用域示例](Developer_images/92.png) 
![bean作用域面试小结](Developer_images/93.png) 

## 15.3 第三方bean管理
![第三方bean管理](Developer_images/94.png)
![示例](Developer_images/95.png) 
![第三方bean管理,统一配置在config中](Developer_images/96.png)

## 15.4 springboot自动配置原理
![自动配置方案1](Developer_images/97.png) 
![自动配置方案2](Developer_images/98.png) 
![自动配置小结](Developer_images/99.png) 

## 15.5 自动配置源码
![自动配置源码](Developer_images/100.png) 
![自动配置源码](Developer_images/101.png) 
![自动配置源码@condition注解](Developer_images/102.png)
![小结面试题](Developer_images/103.png)

## 15.6 自定义starter
![自定义starter](Developer_images/104.png) 
![自定义starter](Developer_images/105.png) 
注意：
@component是扫描当前包及其1级子包下的所有组件，注入到ioc容器中，@Configuration注解表示这是一个配置类，@Bean注解表示这是一个bean，@Conditional注解表示这是一个条件注解，只有满足条件的bean才会被加载。
总结：将第三方所需要的jar包封装成starter，然后在项目的denpendencies中引入starter，就可以使用第三方bean了。注意resources文件夹下的META-INF/spring.factories文件，里面配置了starter的bean，以及条件注解，名称需要模仿其他的注入bean



## 16 Meavn高级
## 16.1 分模块设计
![分模块设计小结](Developer_images/106.png) 
将相关的业务拆分出来，新构建标准的包，通过在pom.xml文件中设置引用设计好的包，实现模块化

## 16.2 meavn继承
![小结，类似于yml配置的继承关系 ](Developer_images/107.png) 
![版本锁定](Developer_images/108.png)
![自定义版本属性](Developer_images/109.png)
![小结，自定义版本属性](Developer_images/110.png)

## 16.3 聚合
![聚合细节，在parent中的pom里面写聚合的子模块](Developer_images/111.png) 
![小结](Developer_images/112.png) 

## 16.4 私服
![私服资源上传与下载1](Developer_images/113.png) 
![私服资源上传与下载2](Developer_images/114.png)  
![私服资源上传与下载3,s](Developer_images/115.png) 
上传的时候，步骤代码需要在meaven的setting.xml文件中配置，下载的时候需要在pom.xml文件中配置私服的地址，然后执行maven命令下载资源。
![从私服下载](Developer_images/116.png) 
...
## 2.2 vue创建
![vue创建](Developer_images/117.png)
![vue安装](Developer_images/118.png)
![vue目录结构](Developer_images/119.png)
![vue项目启动](Developer_images/120.png)

![vue项目开发流程1](Developer_images/121.png)
![vue项目开发流程2](Developer_images/122.png)

## 2.3 API风格
![API风格](Developer_images/123.png)

## 3 前端打包部署
## 3.1 部署Nginx
![前端项目打包](Developer_images/124.png)
![Nginx部署](Developer_images/125.png)
指令如下，这里面需要重新配置config.conf文件,目的是连接到后端的端口，并指定静态文件目录，这里重写了连接api地址，
![反向代理Nginx配置](Developer_images/126.png)
- nginx.exe -s reload  ## 重启nginx，在exe文件目录下启动cmd，执行此命令
![小结](Developer_images/127.png)


# 项目部署
## 1 linux
## 1.1 基本指令
![linux目录结构](Developer_images/128.png)
![目录操作指令](Developer_images/129.png)
![文件操作指令](Developer_images/130.png)
![拷贝移动指令](Developer_images/131.png)
![压缩与解压](Developer_images/132.png)
![文本编辑指令](Developer_images/133.png)
![查找指令find](Developer_images/134.png) 
![查找指令grep](Developer_images/135.png)


## 1.2 软件安装
![jdk软件安装](Developer_images/136.png)
![mysql软件安装1](Developer_images/137.png)
![mysql软件安装2](Developer_images/138.png)
![防火墙开关配置](Developer_images/139.png)
![nginx软件安装](Developer_images/140.png)

## 1.3 项目部署
![前端项目部署](Developer_images/141.png)
![后端项目部署](Developer_images/142.png)


## 2 docker
## 2.1 docker简介
![docker简介](Developer_images/143.png)
![docker命令解读](Developer_images/144.png)

## 2.2 docker常见命令
docker exec -it 容器名 /bin/bash 进入容器内部
docker ps -a 列出所有容器
![docker常见命令](Developer_images/145.png)


## 2.3 docker数据卷
由于在容器里面不能使用vim编辑文件，所以需要使用数据卷，在宿主机上面创建相关文件，然后编译文件，让容器内部的文件与宿主机的文件进行挂载，这样，在容器里面就可以使用宿主机vim编辑后的文件了。
![docker数据卷介绍](Developer_images/146.png)
![docker数据卷指令](Developer_images/147.png)
![docker数据卷小结](Developer_images/148.png)
数据卷自动创建指令：
``` 
docker run -d --name nginx -p 80:80 -v html:/usr/share/nginx/html nginx:1.20.2  
<!-- html是本地目录 ，可以通过docker volume inspect html查看自动创建的卷信息以及存放位置，/usr/share/nginx/html是容器内的目录 -->

```
![本地目录卷挂载，细看下注意](Developer_images/149.png)
![mysql本地挂载实战，挂载数据库目录，初始化，配置脚本](Developer_images/150.png)
![mysql数据卷挂载说明](Developer_images/151.png)
![mysql数据卷挂载小结](Developer_images/152.png)

## 2.4 docker自定义镜像
![dockerfile常见镜像指令](Developer_images/153.png)

## 2.5 docker网络
![docker网络指令](Developer_images/154.png)

## 2.6 docker部署服务器项目
![docker部署服务器项目流程，后端](Developer_images/155.png)
![docker部署服务器项目流程，前端](Developer_images/156.png)

## 2.7 compose部署
通过配置yml文件，来决定容器如何配置的，一次性配置多个容器，并启动
![docker compose的指令](Developer_images/163.png)
![compose部署流程](Developer_images/157.png)
![compose部署所需要的文件，前后端文件，以及sql脚本](Developer_images/158.png)
![compose部署同一网络](Developer_images/159.png)
![compose部署mysql的yml](Developer_images/160.png)
![compose部署后端telas的yml](Developer_images/161.png)
![compose部署前端nginx的yml](Developer_images/162.png)
![compose运行命令](Developer_images/164.png)
